...
SQLite는 SQL을 직접 실행하지 않고, 가상머신(VDBE)을 통해 중간 명령어 형태로 실행.<br>
이는 경량화뿐 아니라 구조 단순화, 유지보수, 확장성 확보를 위한 방식.
...
디스어셈블 결과용
```bash
nm build/config.o | less
nm build/executor.o | less
nm build/lexer.o | less
nm build/main.o | less
nm build/parser.o | less
nm build/schema.o | less
nm build/storage.o | less
```
실제 기계어를 어셈블리 형태
```bash
otool -tvV build/config.o | less
```
